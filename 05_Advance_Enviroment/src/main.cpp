...
size_t animMainCharacterIndex = 1;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// MainCharacter
	modelMainCharacter.loadModel("../models/mayow/personaje2.fbx");
	modelMainCharacter.setShader(&shaderMulLighting);
	
	// Guardian
	modelSoldierEnemy.loadModel("../models/boblampclean/boblampclean.md5mesh");
...
bool processInput(bool continueApplication) {
	...
	if (glfwJoystickPresent(GLFW_JOYSTICK_1) == GL_TRUE) {
		...
		if(fabs(axes[1]) > 0.2){
			...
			animMainCharacterIndex = 0;
		}if(fabs(axes[0]) > 0.2){
			modelMatrixMainCharacter = glm::rotate(modelMatrixMainCharacter, glm::radians(-axes[0] * 0.5f), glm::vec3(0, 1, 0));
			animMainCharacterIndex = 0;
		}
...
void renderSolidScene(){
	...
	*******************************************/
	...
	modelMatrixMainCharacter[3][1] = -GRAVITY * tmv * tmv + 3.5 * tmv + terrain.getHeightTerrain(modelMatrixMainCharacter[3][0], modelMatrixMainCharacter[3][2]);
	tmv = currTime - startTimeJump;
	if(modelMatrixMainCharacter[3][1] < terrain.getHeightTerrain(modelMatrixMainCharacter[3][0], modelMatrixMainCharacter[3][2])){
		isJump = false;
	...
	modelMatrixMainCharacterBody = glm::scale(modelMatrixMainCharacterBody, glm::vec3(0.021f));
	modelMainCharacter.setAnimationIndex(animMainCharacterIndex);
	modelMainCharacter.render(modelMatrixMainCharacterBody);
	animMainCharacterIndex = 1;
...
void applicationLoop() {
	...
	modelMatrixMainCharacter = glm::translate(modelMatrixMainCharacter, glm::vec3(13.0f, 0.05f, -5.0f));
	...
	*******************************************/
	...
	while (psi) {
		...
		glm::mat4 modelMatrixColliderMC = glm::mat4(modelMatrixMainCharacter);
		...
		modelMatrixColliderMC = glm::translate(modelMatrixColliderMC, glm::vec3(modelMainCharacter.getObb().c));
		mainCharCollider.e = modelMainCharacter.getObb().e * glm::vec3(0.021) * glm::vec3(0.787401574);
		mainCharCollider.c = glm::vec3(modelMatrixColliderMC[3]);
		addOrUpdateColliders(collidersOBB, "MainCharacter", mainCharCollider, modelMatrixMainCharacter);
		// Rayo desde el MainCharacter
		glm::mat4 modelMatrixRay = glm::mat4(modelMatrixMainCharacter);
		AbstractModel::RAY ray(10.0f, modelMatrixRay);
		addOrUpdateColliders(lay_Colition_RAY, "RayMainCharacter", ray, modelMatrixMainCharacter);
		...
		soldierCollider.e = modelMainCharacter.getObb().e * glm::vec3(0.021) * glm::vec3(0.787401574);
		...
		//Rayo con Esfera
		for (std::map<std::string, std::tuple<AbstractModel::SBB, glm::mat4, glm::mat4>>::iterator it = collidersSBB.begin(); it != collidersSBB.end(); it++) 
			for (std::map<std::string, std::tuple<AbstractModel::RAY, glm::mat4, glm::mat4>>::iterator jt = lay_Colition_RAY.begin(); jt != lay_Colition_RAY.end(); jt++)
		 ...
		 * Open AL sound data
		 ****************************/
		source0Pos[0] = modelMatrixMainCharacter[3].x;
		source0Pos[1] = modelMatrixMainCharacter[3].y;
		source0Pos[2] = modelMatrixMainCharacter[3].z;
		alSourcefv(source[0], AL_POSITION, source0Pos);

		source1Pos[0] = modelMatrixMainCharacter[3].x;
		source1Pos[1] = modelMatrixMainCharacter[3].y;
		source1Pos[2] = modelMatrixMainCharacter[3].z;
		alSourcefv(source[1], AL_POSITION, source1Pos);
		
		source2Pos[0] = modelMatrixMainCharacter[3].x;
		source2Pos[1] = modelMatrixMainCharacter[3].y;
		source2Pos[2] = modelMatrixMainCharacter[3].z;
		alSourcefv(source[2], AL_POSITION, source2Pos);
...
int main(int argc, char **argv) {
	init(1920, 1020, "Ambiente Avanzado", false);
	applicationLoop();
	destroy();
	return 1;
}